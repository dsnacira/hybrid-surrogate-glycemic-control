...
def _evaluate_candidate(x, scenario, sim_kwargs=None, safety_action_level=False, safety_fitness_level=False):
    ...
    # Legacy fallback: [dose_basal, insulin_duration]
    return simulate_day(
        float(x[0]),
        float(x[1]),
        scenario,
...
def run_ga_only(
    ...
    Robust for the UVA/Padova/simglucose migration:
      - supports 3D chromosome: [basal_rate, insulin_carb_ratio, correction_factor]
      - keeps legacy 2D fallback
      - stores best_out without re-simulation
      - returns monotone best_true_history for AUC/regret analysis
    """
    sim_kwargs = dict(sim_kwargs or {})
